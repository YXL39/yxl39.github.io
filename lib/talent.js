  ...
  checkAndHandleTalentLoss(student) {
    ...
    registerDefaultTalents(game, utils){
      ...
      this.registerTalent({
        ...
        this.registerTalent({
            name: '嬲选手',
            description: '神奇的天赋，不明白有什么作用。（你可以去源代码查看功能）',
            color: '#FFB6C1', // 粉色
            prob: 0.01, // 天赋出现概率（1%）
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    // 确保临时属性存储对象存在
                    if (!student._talent_backup) student._talent_backup = {};
                    ...
                    // 50%概率触发效果
                        // 备份原始能力值（避免重复叠加）
                        if (!student._talent_backup['嬲选手']) {
                            student._talent_backup['嬲选手'] = {
                                thinking: student.thinking,
                            ...
                            // 提升所有能力50%（使用clamp确保数值有效性，兼容现有工具函数）
                            student.thinking = clamp(Number(student.thinking || 0) * 1.5);
                            student.coding = clamp(Number(student.coding || 0) * 1.5);
                            student._talent_state.constmental = clamp(Number(student._talent_state.constmental || student.mental || 50) * 1.5);
                            student.knowledge_ds = Number(student.knowledge_ds || 0) * 1.5;
                            student.knowledge_graph = Number(student.knowledge_graph || 0) * 1.5;
                            student.knowledge_string = Number(student.knowledge_string || 0) * 1.5;
                            student.knowledge_math = Number(student.knowledge_math || 0) * 1.5;
                            student.knowledge_dp = Number(student.knowledge_dp || 0) * 1.5;

                            student._talent_state['嬲选手'] = true;
                            return '嬲选手发动';
                        }
                } catch (e) { console.error('嬲选手 天赋错误', e); }
                return null;
            }
        });
        ...
        this.registerTalent({
            name: '珂朵莉',
            description: '如果幸福有颜色，那一定是被终末之红染尽的苍蓝。',
            color: '#ff0000', // 文本主色（红色基调，配合渐变背景）
            prob: 0.01, // 天赋出现概率（8%，可调整）
            beneficial: true,
            handler: function (student, eventName, ctx) {
                try {
                    ensureTemp(student); // 确保临时存储初始化
                    // 设定触发时机：比赛开始时触发（可根据需求修改事件名）
                    if (eventName !== 'contest_start') return null;

                        if (!student._talent_backup['珂朵莉']) {
                            // 备份原始数据结构能力并翻倍
                            student._talent_backup['珂朵莉'] = { knowledge_ds: student.knowledge_ds };
                            student.knowledge_ds = Number(student.knowledge_ds || 0) * 2.0;
                            student._talent_state['珂朵莉'] = true;
                            return '珂朵莉发动';
                        }
                } catch (e) {
                    console.error('珂朵莉 天赋错误', e);
                }
                return null;
            }
        });
        ...
        this.registerTalent({
            ...
            handler: function (student, eventName, ctx) {
                ...
                try {
                    if (student._talent_backup) {
                        ...
                        for (const k of Object.keys(student._talent_backup)) {
                            ...
                            for (const field of Object.keys(backup)) {
                                // 跳过 constmental，它有自己的处理逻辑
                                if (field === 'constmental') continue;

                                const val = backup[field];
                                // 只记录第一次出现的值（最原始的值）
                                if (!originalValues.hasOwnProperty(field) &&
                                    (typeof val === 'number' ? isFinite(val) : (typeof val !== 'undefined' && val !== null))) {
                    ...
                    // 特殊处理 constmental：恢复到学生的原始 mental 值
                    if (student.mental !== undefined) {
                        student._talent_state = student._talent_state || {};
                        student._talent_state.constmental = Number(student.mental || 50);
                    }
