...
GUOWAIXUANSHOUDESHILIZUIDAZHI = 321;     // 国外老哥的最强实力
DAGONGJICHUSHOUYI = 1000;//打工基础收益
DAGONGNENGLIXISHU = 20;//打工能力收益
ZIJIANXUESHENGSHILI = 1929;//自荐学生最大实力
CHUJINGFAZHI = 1.5;//出境触发特殊事件概率
/* 劝退消耗声誉 */
...
const COUNTRIES = {
    ...
    16: { name: "马来西亚", type: "国家", isNorth: false, baseBudget: NORMAL_PROVINCE_BUDGET, trainingQuality: NORMAL_PROVINCE_TRAINING_QUALITY, costMultiplier: 1.0, bonus: { coding: 1.2, pressureFactor: 0.8 }, pressureFactor: 0.8 },
...
function createDefaultClimate(isNorth, name){
  ...
  if(nameLower.includes('西藏') || nameLower.includes('青海')) return plateau;
    ...
    const southNames = [
        // 中国南方地区（省份、直辖市、特别行政区、地区）
        '海南', '广东', '广西', '云南', '福建',
        '湖南', '湖北', '江西', '贵州', '四川', '重庆',
        '浙江', '香港', '澳门', '中国台湾',
        // 相关国家（东南亚、大洋洲及周边关联国家）
        '新加坡', '越南', '伊朗', '马来西亚', '澳大利亚',
        '印度尼西亚', '印度', '泰国', '韩国', '日本', '菲律宾',
        '柬埔寨' // 补充东南亚典型南方关联国家，贴合数组地理属性
    ];
...
if (typeof window !== 'undefined') {
    ...
    window.COUNTRIES = COUNTRIES; // 新增此行
}
