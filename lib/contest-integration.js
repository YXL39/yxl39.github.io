...
(function(global){
  ...
  function generateInternationalStudents(chineseStudents){
    ...
    if(chinaCount > 0){
        avgAbility = Math.min(Math.round(avgAbility / chinaCount), GUOWAIXUANSHOUDESHILIZUIDAZHI);
        avgThinking = Math.min(Math.round(avgThinking / chinaCount), GUOWAIXUANSHOUDESHILIZUIDAZHI);
        avgCoding = Math.min(Math.round(avgCoding / chinaCount), GUOWAIXUANSHOUDESHILIZUIDAZHI);
        for (const k in knowledgeAvg) knowledgeAvg[k] = Math.min(Math.round(knowledgeAvg[k] / chinaCount), GUOWAIXUANSHOUDESHILIZUIDAZHI);
    } else {
  ...
  function handleMockContestResults(studentStates, config, isPurchased, diffIdx){
      const game = window.game;
      // 记录模拟赛结束的周数，供事件系统检测
      game.lastMockContestFinishedWeek = game.week;
